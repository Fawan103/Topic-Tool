...
if st.button("Fetch Data"):
    ...
    try:
        ...
        for keyword in keywords:
            ...
            items = requests.get(SEARCH_URL, params=search_params).json().get("items", [])
            if not items:
                continue
            ...
            # ==========================
            v_data = requests.get(VIDEO_URL, params={
            ...
            # CHANNEL STATS
            # ==========================
            c_data = requests.get(CHANNEL_URL, params={
                "part": "statistics",
                "id": ",".join(channel_ids),
                "key": API_KEY
            }).json()
        ...
        if results:
            ...
            st.success(f"✅ Found {len(results)} English long-form viral candidates")
