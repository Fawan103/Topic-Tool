    ...
    try:
        ...
        for keyword in keywords:
            ...
            params = {
                ...
                "maxResults": 5,
                "relevanceLanguage": "en",   # ✅ ENGLISH FILTER
                "key": API_KEY,
            }
            ...
            v_data = requests.get(VIDEO_URL, params=v_params).json()
            ...
            c_data = requests.get(CHANNEL_URL, params=c_params).json()
            ...
            for item in items:
                ...
                video_data = video_lookup[video_id]

                # Duration (convert ISO string to seconds)
                dur_iso = video_data["contentDetails"]["duration"]
                duration_secs = int(isodate.parse_duration(dur_iso).total_seconds())
                ...
                subs = int(channel_lookup[channel_id]["statistics"].get("subscriberCount", 0))

                # ✅ FILTER — under 70k subs
                if subs >= 70000:
                    continue
        ...
        if results:
            ...
            st.success(f"✅ Found {len(results)} English videos over 15 mins")
