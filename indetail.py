...
if st.button("Fetch Data"):
    ...
    try:
        ...
        for keyword in keywords:
            ...
            video_lookup = {v["id"]: v for v in v_data.get("items", [])}
            ...
            channel_lookup = {c["id"]: c for c in c_data.get("items", [])}
            ...
            for item in items:
                ...
                video_data = video_lookup[vid]
                channel_data = channel_lookup[cid]

                # Duration in minutes, keep 15+ mins filter
                duration_min = iso_to_minutes(video_data["contentDetails"]["duration"])
                if duration_min < 15:
                    continue
                ...
                # ✅ Subs must be between 50k and 70k
                if subs < 50000 or subs > 70000:
                    continue

                # Upload date (YYYY-MM-DD) from snippet
                published_at = item["snippet"].get("publishedAt", "")
                upload_date = published_at.split("T")[0] if "T" in published_at else published_at
        ...
        if results:
            ...
            st.success(
                f"✅ Found {len(results)} English long-form videos "
                f"(15+ min, 2k+ views, 50k–70k subs)"
            )
...
""")
            ...
            st.warning("No qualifying videos found with the current filters.")
